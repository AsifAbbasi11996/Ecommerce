...
import { getAllCanceledOrders } from '../api/ordersApi' // Ensure correct path
...
const CanceledOrders = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      const data = await getAllCanceledOrders()
      if (data.success !== false) {
        setOrders(data.canceledOrders)
      }
      setLoading(false)
    }
          ...
          {trails.map((style, index) => {
            ...
            return (
              <animated.tr key={order._id} style={style}>
                ...
                <td>{formatDate(order.canceledDate)}</td>
              </animated.tr>
            )
          })}
        </tbody>
